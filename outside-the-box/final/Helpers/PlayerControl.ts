...
import { InvisibleBlock } from "./objects/InvisibleBlock";
...
export class PlayerControl extends EventListener {
    ...
    private collidesWithBlockingBlock(x: number, y: number, blocks: Block[]) {
        return blocks.some((block) => {
            ...
            const collides = block.collidesWithRect(x, y, this.width, this.height);
            if (collides && block instanceof InvisibleBlock) {
                block.notifyPlayerCollision();
            }
